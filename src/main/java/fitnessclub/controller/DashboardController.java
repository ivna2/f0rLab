...
public class DashboardController {
    ...
    public String updateAdmin(@PathVariable Long id,
                              ...
                              RedirectAttributes redirectAttributes) {
        try {
            authService.updateAdmin(id, new UpdateAdminRequest(login, name, email, phone, password));
            redirectAttributes.addFlashAttribute("successMessage", "Администратор обновлен");
        } catch (ResponseStatusException ex) {
            redirectAttributes.addFlashAttribute("errorMessage", ex.getReason());
        }
        return "redirect:/admin/admins";
